...
def make_exe(python_file: str):
    ...
    if os.path.isfile(f'{cur_dir}dist/{python_file_name}.exe'):
        if os.path.isfile(f'{cur_dir}SWReminderTool.exe'):
            os.remove(f'{cur_dir}SWReminderTool.exe')
        shutil.copy(f'{cur_dir}dist/{python_file_name}.exe', f'{cur_dir}SWReminderTool.exe')
